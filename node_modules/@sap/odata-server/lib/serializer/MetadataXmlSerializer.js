...
class MetadataXmlSerializer {
     ...
     */
    _createEdmxElement() {
        return xmlbuilder.create(PREFIX_EDMX + ':' + EDMX, { version: '1.0', encoding: 'UTF-8' })
    ...
    _serializeFacets(facetableElement, facetable) {
        ...
        if (facetable.getSrid && facetable.getSrid() !== null && facetable.getSrid() !== undefined) {
            const type = facetable.getUnderlyingType && facetable.getUnderlyingType()
                || facetable.getType && facetable.getType();
            if ((type === EdmPrimitiveTypeKind.GeographyPoint
                ...
                this._convertEnumValue(
                    annotatedType ?
                        ...
                        constantExpr.getAnnotations()
                            ...
                            .getExpression().getValue().substring(1),
