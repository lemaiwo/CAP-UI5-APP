...
class ExpressionParser {
    ...
    _parseExprValue() {
        ...
        firstTokenKind = [TokenKind.DoubleValue, TokenKind.DecimalValue, TokenKind.IntegerValue]
            .find(kind => this._tokenizer.next(kind));
    ...
    _parsePropertyPathExpr(pathSegments, lastResource) {
        ...
        if (property) {
            ...
            if (property.getType().getKind() === EdmTypeKind.COMPLEX) {
                ...
                if (isCollection) {  // eslint-disable-line no-lonely-if
    ...
    _parseCollectionPathExpr(pathSegments, lastResource) {
        ...
        const tokenKind = [TokenKind.COUNT, TokenKind.LAMBDA_ANY, TokenKind.LAMBDA_ALL, TokenKind.QualifiedName]
            .find(kind => this._tokenizer.next(kind));
        if (tokenKind === TokenKind.COUNT) {
            ...
            this._parseFunction(
                FullQualifiedName.createFromNameSpaceAndName(this._tokenizer.getText()), pathSegments,
                lastResource.getEdmType(), lastResource.isCollection(), false);
        } else {
            throw new UriSyntaxError(UriSyntaxError.Message.TOKEN_KINDS_EXPECTED,
    ...
    _parseFunction(fullQualifiedName, pathSegments, lastType, lastIsCollection, withUnbound) {
        ...
        if (!this._tokenizer.next(TokenKind.CLOSE)) {
            do {
                ...
                const expression = new ExpressionParser(this._edm)
                    .parse(this._tokenizer, this._referringType, this._crossjoinEntitySets, this._aliases);
        ...
        let func;
        if (lastType) {
            func = this._edm.getBoundFunction(fullQualifiedName,
                lastType.getFullQualifiedName(), lastIsCollection, names);
        }
        if (!func && withUnbound) {
    ...
    _parseLambdaRest(isAny, lastResource) {
        ...
        this._lambdaVariables.unshift(
            new UriResource().setKind(ResourceKind.EXPRESSION_VARIABLE)
    ...
    _isComparable(leftType, rightType) {
        return leftType === rightType
            || this._isNumericType(leftType) && this._isNumericType(rightType);
    }
    ...
    _checkEqualityTypes(left, right) {
        ...
        if (leftType.getKind() !== EdmTypeKind.PRIMITIVE
            ...
            throw new UriQueryOptionSemanticError(UriSemanticError.Message.INCOMPATIBLE_TYPE,
                leftType.getFullQualifiedName().toString(), rightType.getFullQualifiedName().toString());
        }
    }
    ...
    _getAddSubTypeAndCheckLeftAndRight(left, right, isSub) {
        ...
        if (this._isNumericType(leftType) && this._isNumericType(rightType)) {
            ...
            if (this._isType(leftType, EdmPrimitiveTypeKind.Double, EdmPrimitiveTypeKind.Single)
                || this._isType(rightType, EdmPrimitiveTypeKind.Double, EdmPrimitiveTypeKind.Single)) {
                return EdmPrimitiveTypeKind.Double;
            } else if (this._isType(leftType, EdmPrimitiveTypeKind.Decimal)
        ...
        if (isSub
            ...
            || this._isType(leftType, EdmPrimitiveTypeKind.Date)
            && this._isType(rightType, EdmPrimitiveTypeKind.Date))) {
    ...
    _getMulDivModTypeAndCheckLeftAndRight(left, right, operatorKind) {
        ...
        } else if ((operatorKind === BinaryOperatorKind.MUL || operatorKind === BinaryOperatorKind.DIV)
            && this._isType(leftType, EdmPrimitiveTypeKind.Duration) && this._isNumericType(rightType)) {
            return leftType;
        }
        throw new UriQueryOptionSemanticError(UriSemanticError.Message.INCOMPATIBLE_TYPE,
            leftType.getFullQualifiedName().toString(), rightType.getFullQualifiedName().toString());
    ...
    _getMulNumericResultType(leftType, rightType) {
        ...
        if (this._isType(leftType, EdmPrimitiveTypeKind.Double, EdmPrimitiveTypeKind.Single)
            ...
            || this._isType(rightType, EdmPrimitiveTypeKind.Int16)) {
            return EdmPrimitiveTypeKind.Int32;
        } else if (this._isType(leftType, EdmPrimitiveTypeKind.Byte)
    ...
    _getModResultType(leftType, rightType) {
        ...
        if (this._isType(leftType, EdmPrimitiveTypeKind.SByte)
            || this._isType(rightType, EdmPrimitiveTypeKind.SByte)) {
            return EdmPrimitiveTypeKind.SByte;
        } else if (this._isType(leftType, EdmPrimitiveTypeKind.Byte)
            || this._isType(rightType, EdmPrimitiveTypeKind.Byte)) {
            return this._isType(leftType, EdmPrimitiveTypeKind.Byte)
                && this._isType(rightType, EdmPrimitiveTypeKind.Byte) ?
    ...
    _checkStructuredTypeCast(type, castType) {
        ...
        FeatureSupport.failUnsupported(FeatureSupport.features.TypeCast,
            castType.getFullQualifiedName().toString(), this._tokenizer.getPosition());
        ...
        if (!isCorrectKind || !castType.compatibleTo(type)) {
            throw new UriQueryOptionSemanticError(UriSemanticError.Message.INCOMPATIBLE_TYPE,
                castType.getFullQualifiedName().toString(), type.getFullQualifiedName().toString());
        }
    }
}
